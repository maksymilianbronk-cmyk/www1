...
const SITES = [
  {
    ...
    desc: "Podróżnicza aplikacja mapowa inspirowana Locus Pro / OsmAnd / Geoportalem: 89 warstw map do przełączania (OSM, topo/outdoor, satelity Esri/Google/NASA/Sentinel, Geoportal WMTS+WMS z ortofotomapą i działkami, mapy krajowe CH/AT/NO/FR/ES/DE, mapy sztabowe „Genshtab”, nakładki szlaków Waymarked, kolei, morza i pogody). Profile map (zapisywane zestawy bazowa+nakładki), tryb porównywania dwóch map z przeciąganym uchwytem, kategoria ostatnio używanych, manager warstw z filtrem, ulubionymi i testem dostępności ⚡. Nakładka Wikimapia w dwóch wariantach (https przez wsrv.nl + bezpośrednia, hash subdomen i0–i15 jak w SAS.Planet) z profilami Google+Wikimapia. Menedżer POI: 9 kategorii, kolorowe piny SVG, notatki, long-press na mapie, lista sortowana odległością, undo usuwania, eksport/import GPX i GeoJSON, foldery punktów z licznikami oraz chmura GitHub — konta użytkowników i baza plików POI w repo (gałąź poi-db, auto-sync przez Contents API, publiczne przeglądanie bazy). Dodawanie własnych map WMS/XYZ z poziomu aplikacji. Cały interfejs na autorskim sprite 40+ ikon SVG (styl stroke, zero emoji). GPS ze śledzeniem, pomiar, wyszukiwarka Nominatim, klucze API per dostawca. Wersja mobilna po audycie: dolny pasek akcji, cele dotykowe 44px+, safe-area, haptyka. PWA gotowe pod aplikację Android; katalog warstw to inżynieria wsteczna pakietów onlinemapsources.xml, AnyGIS i melda.ru (ANALIZA.md).",
    icon: "🧭",
    tag: "narzędzie",
    preview: true,
    meta: {
